...
我们对 `start` 函数进行逐行解析：
  ...
  - `第 201~203 行`：检查 `prefetch` 属性，如果需要预加载，则添加全局事件 `single-spa:first-mount` 监听，在第一个子应用挂载后预加载其他子应用资源，优化其他子应用的加载速度。
  - `第 205~217 行`：首先是 `singularMode` 设置是否为单实例模式（`第 205 行`），然后记录是否启用 `jsSandbox/useJsSandbox` 沙箱运行环境（旧版本需要关闭该选项以兼容 IE。新版本在单实例子应用的情况下默认支持 IE，多实例模式依然不支持 IE）。
...
我们这里需要关注 `entry（子应用的 entry 地址）` 和 `render（子应用被激活时触发的渲染规则）`，这两个参数延迟到 `single-spa` 的子应用激活规则触发后执行，也就是 `registerApplication` 的 `activityFunction` 函数。

我们假设此时我们的子应用已激活，我们来看看这里做了什么。（见下图）

![qiankun](http://shadows-mall.oss-cn-shenzhen.aliyuncs.com/images/blogs/qiankun/4.png)

### 获取子应用资源 - import-html-entry

在子应用（路由规则）激活后，首先在 `第 81~84 行` 中使用了 `import-html-entry` 库从 `entry` 进入加载子应用，加载完成后将返回一个对象（见下图）
