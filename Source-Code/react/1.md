...
进入 `performUnitOfWork` 函数内部，我们省略掉一系列目前不需要关注的函数，首先进入到 `beginWork` 函数（见上图）。
...
接下来就是对子节点的依次更新流程，也就是 `App Component` 对应的 `FiberNode`。依然是 `beginWork` 函数，在 `第 232~246 行` 调用我们的 `App Component` 节点的更新流程。
...
还是 `beginWork` 函数（见上图），进入 `updateHostComponent` 进行 `React Element(FiberNode)` 组件更新阶段。
...
在结束了 `beginWork` 流程后，将调用 `completeUnitOfWork` 函数创建 `真实 DOM 树`。
...
通过调用 `createInstance` 方法（见上图 2）创建真实 DOM（此时还没有插入到文档中），然后将 `DOM 树` 对象挂载到 `FiberNode` 的 `stateNode` 属性上（见上图 1）。
...
在 `createInstance` 内部还调用了 `createElement` 函数创建了真实 `DOM` 节点（见上图 1），然后通过递归遍历 `props` 中的属性（包括 `children`）构建了一棵`真实 DOM 树`（见上图 2）
...
`react-dom` 将在回调函数内部将调用 `insertOrAppendPlacementNodeIntoContainer` 方法对 `FiberNode` 进行遍历。由上图可知该函数会对 `Host` 节点（带有 `html tag` 结构的节点）调用 `appendChildToContainer` 函数进行渲染，其他节点取其 `child` 值进行递归调用。
...
在 `appendChildToContainer` 函数内部，通过 `appendChild` 将 `FiberNode` 上的 `stateNode` （我们在上一步创建好的 `真实 DOM 树`）添加到 `container（#app）`中，然后调用 `componentDidMount` 生命周期钩子函数。

![React](http://shadows-mall.oss-cn-shenzhen.aliyuncs.com/images/blogs/rsc/react/49.png)

到了这一步，页面中就渲染了我们在 `render` 中设置的 `jsx 语法标签`（`Hello World`），我们的渲染流程解析宣告完成！
