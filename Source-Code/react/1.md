...
`updaterContainer` 是一个比较关键的函数，我们来解析一下这个函数做了什么：
  ...
  - `第 23~32 行`：进入渲染主流程，`第 24 行` 将 `Element` 实例（见下图 1）挂载在 `update` 对象上，`第 31 行` 设置 `FiberNode` 实例（见下图）的更新队列（`updateQueue`)。
...
![React](http://shadows-mall.oss-cn-shenzhen.aliyuncs.com/images/blogs/rsc/react/28.png)
  ...
  - `第 36 行`：对 `FiberNode` 的子节点进行遍历，也就是对上一步的 `memoizedState` 中的 `element` 进行进一步的处理（见下图）。
