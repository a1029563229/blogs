...
到这里，`Component` 类的结构已经解析完成了。什么，这就解析完成了？生命周期函数呢？渲染过程呢？一个都还没有看到啊。别着急，由于 `React` 内部的职责划分与不同平台实现，所以这部分根据不同平台的实现被放在了 `react-dom` 或 `react-native` 中，然后以依赖注入的形式参与协作。我们接下来就对我们常用的浏览器端，`react-dom` 中渲染过程以及对组件生命周期的处理进行详细的梳理。在此之前，放张图对本章的 `Component` 进行小结。
...
`FiberNode` 由内部的 `createFiber` 函数进行创建。（这也是 `React` 在 `16` 版本后作出的巨大更新，这个后面我们再展开说）。
...
在构建完了根 `FiberNode` 实例后，`第 40 行` 调用了 `updaterContainer`  函数开始构建整棵 `FiberNode` 树以及完成 `DOM` 渲染。
...
进入 `scheduleUpdateOnFiber` 函数内部，我们省略掉一系列目前不需要关注的函数，首先进入到 `beginWork` 函数（见上图）。
...
然后根据节点的标签类型（`tag`），调用不同的函数进行内部状态更新。

### root 节点更新 - `updateHostRoot`
...
我们第一次进入是 `root` 节点，所以进入到 `updateHostRoot` 函数内部逻辑进行处理。（见上图）
...
### `App Component` 更新流程 - `updateClassComponent`

接下来就是对子节点的依次更新流程，我们这里选取 `App Component` 对应的 `FiberNode` 来进行解析。
...
在本次工作流程的最后一步，将会构建真实 DOM 树。通过调用 `createInstance` 方法（见下图）创建真实 DOM（此时还没有插入到文档中），然后将 `DOM 树` 对象挂载到 `FiberNode` 的 `stateNode` 属性上。

![React](http://shadows-mall.oss-cn-shenzhen.aliyuncs.com/images/blogs/rsc/react/50.png)

由上图可知，在 `createInstance` 内部还调用了 `createElement` 函数创建了真实 `DOM` 节点，然后通过递归遍历 `props` 中的属性（包括 `children`）构建了一棵`真实 DOM 树`（见下图）
...
在 `真实 DOM 树`构建，并且在 `nextChildren` 为 `null` 后将跳出 `workLoopSync`，进入到 `finishSyncRender` 函数，进行节点的渲染工作。

### 渲染真实 DOM

![React](http://shadows-mall.oss-cn-shenzhen.aliyuncs.com/images/blogs/rsc/react/47.png)

内部将调用 `insertOrAppendPlacementNodeIntoContainer` 方法对 `FiberNode` 进行遍历。由上图可知该函数会对 `Host` 节点（带有 `html tag` 结构的节点）进行操作，而对非 `Host` 节点取 `child` 值进行递归调用。

这里进入到 `appendChildToContainer` 函数后，`FiberNode` 上的 `stateNode` 将通过 `appendChild` 添加到 `container（#app）`中（见下图），然后调用 `componentDidMount` 生命周期钩子函数。
...
到了这一步，页面中就渲染了我们在 `render` 中设置的 `jsx 语法标签`，，我们的渲染流程解析宣告完成！
