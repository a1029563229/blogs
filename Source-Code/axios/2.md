...
这个动作不止在发起正式请求前，而且在请求成功和请求失败时都做了一次，只要是被 `cancel` 的请求，都是不会进入到成功或失败回调处理中的。（如下图）
...
上面的代码主要做了两件事情，第一件事情就是将通用的 `headers`（`common headers`）和对应方法的 `headers` 以及原 `headers` 做了一个合并；第二件事情就是在合并完成后，将多余的 `headers` 配置删除。
...
你可以在这里传入一个自定义的请求方法，使用 `fetch` 封装而不是 `XMLHttpRequest`。

你也可以在这里传入一个你封装好的 `mock` 方法，返回的都是本地 `json` 数据，用于 `mock` 数据，这样你就不用额外搭建一个 `mock` 服务啦。

......

言归正传，我们还是来看看 `axios` 默认的两个 `adapter` 吧。
...
这里对 `password` 进行了 `encodeURLComponent` 转移编码，然后再将用户名与密码按照规则拼接后，使用了 `btoa` 转成了 `base64` 编码。
...
比较晚入行接触前端的可能对这些不太清楚，可以参考一下 [XMLHttpRequest 文档](https://developer.mozilla.org/zh-CN/docs/Web/API/XMLHttpRequest)。
...
好了，我们对 `axios` 源码的解读就到这里为止了，`axios` 虽然是目前最流行的网络请求框架，但是源码看起来还是比较清爽易读的，建议大家可以自己按照文章的思路去看看。
