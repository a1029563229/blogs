...
介绍完不同技术栈的构建打包方法后，我们会先介绍如何使用 `Node` 部署发布，然后再介绍如何使用 `Caddy` 方案（对前端更友好的反向代理神器）发布，也就是下面两种方案：
...
```nginx
server {
  listen 9999 # 运行在 9999 端口
  location / {
    root ./micro-app-main; # 主应用目录
    index index.html index.htm; # 入口文件
    try_files $uri $uri/ /index.html @rewrites; # 兼容 history
  }
}
```

配置文件写好后，我们直接运行 `nginx` 命令启动我们的 `nginx` 服务

## 总结

本文还在编辑中，如果对发布部署这块的需求比较紧急，可以参照 [实战案例 - feature-deploy 分支](https://github.com/a1029563229/micro-front-template/tree/feature-deploy) 的 `deployed` 目录下的 `Caddy` 部署方案和 [Caddy 实战教程](https://github.com/a1029563229/Blogs/tree/master/BestPractices/caddy) 来进行部署。